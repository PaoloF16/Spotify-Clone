...
const artistiSceltiPrimo = [...artisti].sort(() => Math.random() - 0.5).slice(0, 20);
...
const artistiSceltiSecondo = [...artistiRimanenti].sort(() => Math.random() - 0.5).slice(0, 20);
...
function popolaCarosello(listaArtisti, carosello) {
  listaArtisti.forEach((artista) => {
    fetch(searchLink + artista)
      ...
      .then((data) => {
        if (!data.data || data.data.length === 0) return;
        const albumFiltrati = data.data.filter((album) => album.cover_medium && !albumGiaUsati.has(album.id));
        if (albumFiltrati.length === 0) return;
        const album = albumFiltrati[Math.floor(Math.random() * albumFiltrati.length)];
...
const popolaPiccoleCards = function (album, wrapper) {
  if (document.querySelectorAll("#small-cards-wrapper .col").length === 8) {
    return;
  }
  wrapper.innerHTML += `
          <div class="col">
