...
const searchMixedSuggestions = async (query) => {
  try {
    ...
    const mixedResults = [...uniqueArtists.slice(0, 4).map((item) => ({ type: "artist", item })), ...uniqueSongs.slice(0, 4).map((item) => ({ type: "song", item }))];
...
const createAlbumArray = async () => {
  ...
  const promises = artisti.map(async (artista) => {
    try {
      ...
      const artistaMatch = searchData.find((item) => item.artist?.name?.toLowerCase().includes(artista.toLowerCase()));
